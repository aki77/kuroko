...
アプリメニュー「表示 > デバッグウィンドウを開く」で、裏側の処理過程をリアルタイムに可視化する専用ウィンドウを開ける。オーバーレイの提案表示・Status には一切影響しない（副作用ゼロの読み取り専用ビュー）。

表示される内容:
- 文字起こし取り込み状況（監視中ファイルの切替・**直近発話の話者名+本文（末尾30件）**・パースエラー）
- 処理パイプラインの段階（`no-meeting`/`idle`/`waiting`/`querying`/`error` の遷移・手動トリガー・needsCode判定）
