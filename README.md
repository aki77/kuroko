...
フッタの履歴ナビ（`◀ N/N ▶`）では同一会議中に生成された提案ラウンドを1件ずつめくって振り返れる。フッタの🕘ボタン（または`⌘⇧L`）を押すと、これまでの話題・要約・時刻を時系列（古い→新しい）で縦一覧できる「要約の流れ」オーバーレイが開く。開いた直後は最新（最下部）へスクロールし最新行を強調表示する。一覧を開いたまま新しい提案ラウンドが確定すると行が追加され（ライブ追従）、最下部付近を見ているときは新着行まで自動追従し、過去を遡って閲覧中はスクロール位置を維持する。行をクリックするとそのラウンドのめくり表示へジャンプして一覧を閉じる。`Esc`・✕ボタン・`⌘⇧L`で閉じる（🕘ボタンは開いた一覧に覆われるため、閉じるのはこれらの手段を使う）。一覧は topic・discussion・時刻のみを表示し（次に話すべきこと・FROM THE WEB/CODEは出さない）、会議切替時にクリアされる（同一会議中のみ保持・非永続化）。
