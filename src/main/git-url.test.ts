...
test("buildGithubBlobUrl: 複数コロン(path側にコロン残り) -> null", () => {
  assert.equal(
    buildGithubBlobUrl("src/foo.ts:12:34", repo, "/x", clean, fresh),
    null,
  );
});
...
test("buildGithubBlobUrl: '..'含み -> null", () => {
  assert.equal(
    buildGithubBlobUrl("../secret:1", repo, "/x", clean, fresh),
    null,
  );
});
...
test("buildGithubBlobUrl: 空白含み -> null", () => {
  assert.equal(
    buildGithubBlobUrl("src/foo bar.ts:1", repo, "/x", clean, fresh),
    null,
  );
});
...
test("buildGithubRefUrl: カンマ区切り複数範囲 -> 先頭範囲のURL", () => {
  assert.equal(
    buildGithubRefUrl(
      "app/models/review.rb:11-12, 29-33, 16",
      repo,
      "/x",
      clean,
      fresh,
...
test("buildGithubRefUrl: path側にコロン残り(不正ref) -> null", () => {
  assert.equal(
    buildGithubRefUrl("a.ts:1:2, 3", repo, "/x", clean, fresh),
    null,
  );
});
...
test("buildGithubRefUrl: 絶対パス -> null", () => {
  assert.equal(
    buildGithubRefUrl("/etc/x:1, 2", repo, "/x", clean, fresh),
    null,
  );
});
...
test("buildGithubRefUrl: '..'含み -> null", () => {
  assert.equal(
    buildGithubRefUrl("../secret:1, 2", repo, "/x", clean, fresh),
    null,
  );
});
