...
test("findUnreachableUrls: 404/410のurlは含まれる", async () => {
  const result = await findUnreachableUrls(["https://a.example", "https://b.example"], {
    fetchImpl: fakeFetch({ "https://a.example": 404, "https://b.example": 410 }),
  });
...
test("findUnreachableUrls: HEADが405ならGETにフォールバックし、GETのstatusで判定する", async () => {
  ...
  const result = await findUnreachableUrls(["https://a.example"], { fetchImpl });
  assert.equal(result.has("https://a.example"), true);
...
test("isUrlUnreachable: HEADが405でGETがハングしても合計timeoutMs以内に解決する(回帰防止)", async () => {
  ...
  assert.equal(result, false);
  assert.ok(elapsed < timeoutMs * 1.3, `elapsed=${elapsed}ms がtimeoutMsを大きく超えている`);
});
...
test("findUnreachableUrls: 重複urlは1回だけ問い合わせて両方に反映される", async () => {
  ...
  const result = await findUnreachableUrls(["https://a.example", "https://a.example"], {
    fetchImpl,
  });
